...
export default function Quiz({ props, data }: { props: Props; data: Data }) {
    const { title, description, ctaStart, steps, result } = props;
    const { index } = data;
    ...
    return (
        <div class="flex w-full h-full py-10 justify-center items-center container relative min-h-96">
            {index == 0
                ? (
                    <div class="flex h-full m-auto flex-col gap-5 px-2 lg:px-0">
                        <h1 class="text-xl lg:text-3xl text-center ">{title}</h1>
                        {description && (
                            <span class={"text-center max-w-2xl mx-auto"} dangerouslySetInnerHTML={{ __html: description }}></span>
                        )}
                        <button
                ...
                : steps[index - 1] ? (
                    <div class="flex h-full flex-col gap-5 px-2 lg:px-0 ">
                        <h1 class=" text-xl lg:text-3xl">{steps[index - 1].question}</h1>
                        <ul class={"flex flex-col lg:flex-row flex-wrap gap-2"}>
                            {steps[index - 1].options.map((item) => (
                                <li hx-get={useSection({ props: { data: { index: index + 1 } } })}
                                    hx-target="closest section"
                                    hx-swap="outerHTML"
