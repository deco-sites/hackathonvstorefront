    ...
    function spin(spinAngleStart: number, spinTimeTotal: number): void {
      ...
      function rotate(): void {
        ...
        if (spinTime >= spinTimeTotal) {
          ...
          const index = Math.floor(((2 * Math.PI) - finalAngle) / arc) %
            cupons.length;
        ...
        {activeCupom == undefined
          ? (
            <>
              <div class={"w-full lg:w-2/4 flex flex-col gap-4 lg:gap-8"}>
                ...
                <button
                  id="girar"
                  class="border border-primary px-8 py-4 uppercase text-primary hover:bg-primary hover:text-white font-bold duration-300 mt-3 max-w-48"
                >
                  {ctaStart}
                </button>
              </div>
              <div class="flex justify-center items-center w-full lg:w-2/4">
                <div class="relative">
                  ...
                  <canvas
                    id="canvas"
                    class="max-w-[300px] lg:max-w-[500px]"
                    width="500"
                    height="500"
                  >
                  </canvas>
          ...
          : (
            <div class="flex justify-center items-center container min-h-[400px] flex-col gap-8 max-w-[700px]">
              <h2 class="text-xl lg:text-3xl text-center ">{titleSuccess}</h2>
              <span
                class="text-xl lg:text-xl text-center "
                dangerouslySetInnerHTML={{
                  __html: descriptionSucces.replace("CUPOM", activeCupom),
                }}
              >
              </span>
              <button
                ...
                hx-swap="outerHTML"
                hx-target="closest section"
                class="px-8 py-4 bg-primary text-white uppercase font-bold hover:opacity-85"
              >
                Tentar novamente
              </button>
            </div>
